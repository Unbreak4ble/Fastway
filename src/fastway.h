...
namespace FastWay {
   ...
   std::vector<std::vector<components::xy>> start(std::vector<std::vector<int>> lab, void(*callback)(std::vector<std::vector<int>>)){
     auto newlab = lab;
     ...
     for(int i=0; i<results[0].size(); i++){
       ...
       if(needReturn){
         ...
         auto otherLabPos = start(newlab, callback);
   ...
   std::vector<std::vector<int>> run(std::vector<std::vector<int>> lab, void(*callback)(std::vector<std::vector<int>>)=nullptr){
     auto results = start(lab, callback);
   ...
   std::vector<std::vector<int>> run(std::string filename, void(*callback)(std::vector<std::vector<int>>)=nullptr){
     std::string content = components::readFile(filename);
     auto lab = components::interpret(content);
     return run(lab, callback);
   }
